...
class Preprocessor:
    def __init__(self, config):
        ...
        self.nlp = spacy.load('{}_core_web_sm'.format(config.lang))
    ...
    def check_text(self, tokenized_text, source_text):
        if self.config.bert_path in ['YOUR_ROBERTA_LARGE_PATH', 'roberta-base']:
    ...
    def align_index(self, sentences):
        ...
        for sentence in sentences:
            ...
            if self.config.bert_path in ['YOUR_ROBERTA_LARGE_PATH', 'bert-base-uncased']:
    ...
    def repack_unknow(self, source_sequence):
        ...
        if start < len(source_sequence):
            ...
            new_tokens += token
        return new_tokens
    #-------------------------------semantic : speaker + discourse structure------------------------#
    def get_semantic_adj(self,speakers,dialog_len): 
        # semantic_adj = []
        s = torch.zeros(dialog_len, dialog_len, dtype = torch.long) # （N,N） 
    ...
    def get_structure_adj(self,replies,dialog_len):
       
        # Define the structure of dialogue threads based on replies
        threads = []
        current_thread = -1
        for r in replies:
            if r == 0:
        ...
        s = torch.zeros(dialog_len, dialog_len, dtype = torch.long)
        ...
        for i in range(dialog_len):
            for j in range(dialog_len):
                if i == j:
                    ...
                    s[i,j] = 2
                    s[j,i] = 3
                elif threads[i]==threads[j] or threads[i]==-1 or threads[j] == -1:
                    if s[i,j] == 0:
                        s[i,j] = 4
                    if s[j,i] == 0:
                        s[j,i] = 5   
        ...
        for document in dataset:
            ...
            for i,reply in enumerate(replies):
                if reply == -1:
                    ...
                    windows_sentence.append(sentences[i])   
                if i == len(replies) - 1:
                    # windows_sentence = [ thread_w for t in windows_sentence for  thread_w in t]
                    windows_sentence = [ tw for tw in  windows_sentence]
                    thread_sentence.append(windows_sentence)
            ...
            thread_sentences = [root_sentence + thread_sentence[i] for i in range(1,len(thread_sentence))]
            cnt = 0
            sent_idx = [0]
            thread_sents_index = []
            for i,sents in enumerate(thread_sentences):
                for j,sent in enumerate(sents):
                    if j==0:
                        continue
                    cnt+=1
                    sent_idx.append(cnt) 
                thread_sents_index.append(sent_idx)
                sent_idx = [0]
            thread_contexts = []
            for inner_list in thread_sentences:
                flat_inner_list = [item for sublist in inner_list for item in sublist]
                thread_contexts.append(flat_inner_list)
                #=============================================================================#
            ...
            thread_tokens =  [[self.config.cls] + w + [self.config.sep] for w in thread_contexts]
            ...
            flatten_thread_tokens = [w for line in thread_tokens for w in line]
            ...
            origion_utterance_span = [] #[(0, 20), (21, 56), (57, 95), (96, 114), (115, 132), (133, 155), (156, 205), (206, 263), (264, 292), (293, 310)]
            start_index = 0
            for sentence in sentences:
            ...
            spk_adj = self.get_semantic_adj(speakers,len(sentences))#[10,10]
            dis_adj = self.get_structure_adj(replies,len(sentences))
            ...
            for i, sentence in enumerate(sentences):
                ...
                pos_tag = self.POS_generate(str_sentence) #[,len(sentence)]
                pos_tags.append(pos_tag)
                matrix = self.dependency_adj_matrix(str_sentence)
                adj_matrix.append(matrix)#[len(sentence),len(sentence)]
            #----------------------------------------------------# 
            ...
            res.append((doc_id, input_ids, input_masks, input_segments, sentence_length, nsentence_ids, utterance_index, token_index, 
                        ...
                        new_triplets, pairs, entity_lists, relation_lists, polarity_lists,spk_adj,dis_adj,adj_matrix,pos_tags, thread_input_ids,thread_input_masks,thread_input_segments,
                        utterance_spans,thread_sents_index,origion_utterance_span,nsentence_ids_raw,sentence_length_raw))
        return res

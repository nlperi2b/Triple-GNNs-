...
class GraphAttentionLayer(nn.Module):
    ...
    def __init__(self, in_features, out_features, dropout, alpha, concat=True, relation = False, num_relation=-1,relation_coding='hard',relation_dim=50):
        ...
        if self.relation:
            if relation_coding=='hard':
                emb_matrix = torch.eye(num_relation).to(torch.device("cuda"))  # num_relation 只有relation=True时 有效
    ...
    def forward(self, h, adj):
        ...
        a_input = self._prepare_attentional_mechanism_input(Wh).to(torch.device("cuda"))  # (B, N, N, 2*D_out)

        if self.relation:
            long_adj = adj.clone().type(torch.LongTensor).to(torch.device("cuda"))
            # pdb.set_trace()
        ...
        # a_input = a_input.to(torch.device("cuda") ) #[4,9,9,2054]
...
class RGAT(nn.Module):
    def __init__(self, args, nfeat, nhid, dropout = 0.2, alpha = 0.2, nheads = 2, num_relation=-1):
        ...
        for i, attention in enumerate(self.attentions):
            self.add_module('attention_{}'.format(i), attention) #self.add_module(name, module),为每个子模块指定了一个名称。

from src.model_utils import RGAT, DiffLoss
import torch
import torch.nn as nn
import torch.nn.functional as F
import pdb

"""Based on: https://github.com/something678/TodKat """
class DualGATs(nn.Module):
    ...
    def __init__(self, args):
        ...
        for _ in range(args.gnn_layers):
            ...
            DisGAT.append(RGAT(args, args.hidden_dim, args.hidden_dim, dropout=args.dropout, num_relation=6))
        # relation 
        ...
        in_dim = args.hidden_dim *2 + args.emb_dim #模型的特征输出维度：spkGAT + disGAT + feature
        # output mlp layers
    ...
    def forward(self, utterance_features, semantic_adj, structure_adj):
        '''
        :param utterance_features: (B, N, emb_dim)
        :param xx_adj: (B, N, N)
        :return:
        '''
        ...
        H = torch.cat([H[-3],H[-2],H[-1]], dim = 2) #(B, N, 2*hidden_dim+emb_dim) 
       
        logits = self.out_mlp(H) #返回(B,N,hidden_dim)
        return logits, self.beta * (diff_loss/self.args.gnn_layers) #logits:[B,max_U,hidden_State], 2：loss值
